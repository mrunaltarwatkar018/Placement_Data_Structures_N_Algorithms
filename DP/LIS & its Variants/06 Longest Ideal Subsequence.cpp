/* Leetcode Problem No.: 2370. Longest Ideal Subsequence  */
...
/************************************************************** C++ ******************************************************************/
// Approach-1 (Using LIS code eactly) - TLE      74 / 85 testcases passed
// T.C : O(n^2)
// S.C : O(n)
class Solution {
public:
...
// Approach-2 (Optimised - Check only those characters for s[i] such that abs(ch - s[i]) <=k)
// T.C : O(n)
// S.C : O(1)
class Solution {
public:
    int longestIdealString(string s, int k) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            int longest = 0;
            for(int j = left; j <= right; j++) { // left = 0 in wrost case, right = 25 in wrost case
