/*    Leetcode Problem No.: 1026. Maximum difference between node and its ancestor    */
...
/*
    Approach Summary (Optimal Approach) - 
        The key function is maxAncestorDiff, which initializes minimum and 
        maximum values and then calls the recursive function findMaxDiff to 
        compute the maximum difference between any two nodes in the binary tree.
        
        The findMaxDiff function traverses the tree recursively, updating the 
        minimum and maximum values encountered along the way, and computes the 
        maximum difference for each subtree. 

        The final result is the maximum difference obtained across all subtrees, 
        and it represents the maximum absolute difference between any two nodes 
...
/*
    YOUTUBE VIDEO ON THIS Qn : https://www.youtube.com/watch?v=cvekQzmBdcs
*/