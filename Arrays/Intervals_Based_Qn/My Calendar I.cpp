/* Leetcode Problem No.: 729. My Calendar I  */
...
/*
    Approach 2: Using set with lower_bound (O(n log n) Time, O(n) Space)
            ...
            This approach uses a set of pair to store intervals, automatically sorted by    
            start time. The lower_bound function is used to find the first interval starting 
            after the new booking and checks for overlap with both the previous and next 
            intervals.

            This method is more efficient, with O(n log n) time complexity due to the 
            logarithmic insertion and search in the set.
