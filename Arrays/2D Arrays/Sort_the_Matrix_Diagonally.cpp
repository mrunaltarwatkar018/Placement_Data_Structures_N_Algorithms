/* Leetcode Problem No.: 1329. Sort the Matrix Diagonally  */
...
/*
    ...
    public:
        vector<vector<int>> diagonalSort(vector<vector<int>>& mat) {
            ...
            // step 1: store diagonal elements in a map
            for( int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
            ...
            // step 3: now, put them back in sorted order
            for (int i = m -1; i >= 0; i--) {
                for (int j = n -1; j >= 0; j--) {
                    mat[i][j] = mp[i-j].back();
                    mp[i-j].pop_back();
...
/*
    YOUTUBE VIDEO ON THIS Qn : https://www.youtube.com/watch?v=mNWwJQ7_z4Q
*/