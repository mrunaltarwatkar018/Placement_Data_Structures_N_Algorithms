...
/*
    Approach
        ...
        This problem is not as stated in description,
        (we don't have to make image 1 == image 2)

        We have to check all the matrix of array img1 with img2 with every overlapping 
        within the range of -n+1 to n-1 and find out count of 1 with every overlapping.

        And when we get the maximum count of 1 by trying all the possible overlapping 
        we will return the maximum count of 1.

        (This question task is to find the overlapping range not the index element).
*/
class Solution {
public:

    // This function check the is the img1 and img2 are equal to 1;
    int overLap(vector<vector<int>>& img1, vector<vector<int>>& img2,int row, int col){
        ...
        for(int i = 0; i < n; i++){
            for(int j = 0; j< n; j++){
                ...
                //updated index of the img2 for checking the 1 in both the matrix
                int img2_i = i+row;
                int img2_j = j + col;
    ...
    int largestOverlap(vector<vector<int>>& img1, vector<vector<int>>& img2) {
        ...
        for(int row = -n+1; row < n; row++){
            for(int col = -n+1; col < n; col++){
                int count = overLap(img1,img2,row,col);
...
/*
    YOUTUBE VIDEO ON THIS Qn : https://www.youtube.com/watch?v=3--u8aPXzWs
*/